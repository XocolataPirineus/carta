  ...
  <Layout>
    {data.allCartaXlsxHoja1.group.map(({ fieldValue, edges }) => (
      <div key={fieldValue}>
        ...
        >
          {edges[0].node.titulo_esp} / {edges[0].node.titulo_cat}
        </h3>
        <hr style={{ borderTop: "1px solid black" }} />
        {edges.map(({ node }) => (
          <Articulo data={node} key={node.id} />
        ))}
      </div>
    ))}
  </Layout>
...
class Articulo extends React.Component {
  ...
  componentDidMount() {
    this.setState({
      isSaved: JSON.parse(window.localStorage.getItem(this.key)),
    })
  }
    ...
    return (
      <div>
        ...
        >
          ...
          &nbsp;
          <strong>
            {this.state.data.castellano}
            {this.state.data.catalan ? " /" : ""}
          </strong>{" "}
          {this.state.data.catalan}
